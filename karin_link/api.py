...
def create_app(
    ...
    allowed_origins = os.environ.get("CORS_ORIGINS", "http://localhost:7800,http://127.0.0.1:7800").split(",")
    app.add_middleware(
        CORSMiddleware,
    ...
    async def list_devices(device_id: str = Depends(verify_token)) -> list:
    ...
    async def list_rooms(device_id: str = Depends(verify_token)) -> list:
