...
object AutoPlayManager {
    ...
    fun findNextEpisodeUrl(currentUrl: String, currentEpisodeNumber: Int): String? {
        ...
        val padded = nextNumber.toString().padStart(2, '0')

        val patterns = listOf(
            Regex("""(episodio-)\d+""") to "episodio-$nextNumber",
            Regex("""(-ep)\d+""") to "-ep$nextNumber",
            Regex("""(episode-)\d+""") to "episode-$nextNumber",
            Regex("""(episode/)\d+""") to "episode/$nextNumber",
            Regex("""(ep/)\d+""") to "ep/$nextNumber",
            Regex("""(ep)\d+(\.html)""", RegexOption.IGNORE_CASE) to "ep$nextNumber$2",
            Regex("""(/capitulo-)\d+""") to "/capitulo-$nextNumber",
            Regex("""(-cap-)\d+""") to "-cap-$nextNumber",
            Regex("""(_ep_?)\d+""") to "_ep$nextNumber",
            Regex("""(part=)\d+""") to "part=$nextNumber",
            Regex("""(\?ep=)\d+""") to "?ep=$nextNumber",
